...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-slate-100 text-slate-900" x-data="{ mobileSidebarOpen: false }" @keydown.escape.window="mobileSidebarOpen = false">
        @php
            ...
            $sidebarItems = match ($currentRole) {
                ...
                'dosen' => [
                    ...
                    ['label' => 'Mahasiswa', 'route' => 'dosen.mahasiswa.index', 'icon' => 'students'],
        ...
        <div class="min-h-screen bg-slate-100">
            <div class="flex min-h-screen">
                ...
                <aside class="fixed inset-y-0 left-0 z-50 w-72 transform bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white transition-transform duration-300 ease-in-out xl:hidden"
                    ...
                    <div class="flex-1 px-4 py-5">
                        ...
                        <nav class="mt-4 space-y-2">
                            @foreach ($sidebarItems as $item)
                                @php $isActive = request()->routeIs($item['route']) || ($item['route'] === 'dashboard' && request()->routeIs('dashboard')); @endphp
                                <a href="{{ route($item['route']) }}" @click="mobileSidebarOpen = false" class="group flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition {{ $isActive ? 'bg-white/12 text-white shadow-lg ring-1 ring-white/10' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="hidden md:block">
                                    @isset($header)
                                        {{ $header }}
                                    @else
                                        <div class="text-sm font-semibold text-slate-700">{{ $appName }}</div>
                                    @endisset
                                </div>
                            </div>
